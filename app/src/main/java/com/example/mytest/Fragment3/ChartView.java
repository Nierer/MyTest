...
public class ChartView extends View {
    ...
    //x轴各个坐标点水平间距
    private float interval = dpToPx(40);
    //背景颜色
    ...
    //折线对应的数据
    private Map<Float, Float> mSpinnerValue = new HashMap<>();
    //点击的点对应的X轴的第几个点，默认1
    ...
    private void init(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            if (attr == R.styleable.ChartView_xy_line_color) {
                ...
                interval = (float) array.getDimension(attr, TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_PX, interval, getResources().getDisplayMetrics()));
    ...
    protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
        if (changed) {
            ...
            float textYWdith = getTextBounds( "0000", mXYTextPaint).width();
            ...
            xValueRect = getTextBounds("0000", mXYTextPaint);
            //X轴文本高度
        ...
        selectIndex = (int)getSelectIndexFromShowType(mShowPositionType);
    ...
    private void drawBrokenPoint(Canvas canvas) {
        ...
        for (int i = 0; i < flag_2; i++) {
           // double value=mXData.get(i)-Math.floor(mXData.get(i)) ;
            float x = (float)(mXInit +mXData.get(i)*interval);
            float y =0;
            if(flag==1) {
                y = mYOri - mYOri / 2 + dpToPx(18) - mYOri * (1 - 0.1f) * mSpinnerValue.get(mXData.get(i)) / mYData.get(mYData.size() - 1) / 2;
            }
            else if(flag==2){
                y = mYOri - mYOri / 6 + dpToPx(5) - mYOri * (1 - 0.1f) * mSpinnerValue.get(mXData.get(i)) / mYData.get(mYData.size() - 1)/6*5 ;
            }
            else{
                y = mYOri  - mYOri * (1 - 0.1f) * mSpinnerValue.get(mXData.get(i)) / mYData.get(mYData.size() - 1) ;
            }
            //绘制选中点
            if (i == selectIndex - 1) {
                mSpinnerLinePaint.setStyle(Paint.Style.FILL);
    ...
    private void drawXY(Canvas canvas) {
        ...
        path = new Path();
        //整个长度
        float xLength = mXInit + interval * (flag_2 - 1) + (width - mXOri) * 0.1f;
        if (xLength < width)
            xLength = width;
    ...
    private void clickAction(MotionEvent event) {
        ...
        for (int i = 0; i < mXData.size(); i++) {
            ...
            //X轴刻度
            String text = String.valueOf(mXData.get(i));
            Rect rect = getTextBounds(text, mXYTextPaint);
            x = mXInit + interval * i;
     ...
     */
    private float getSelectIndexFromShowType(int showPositionType) {
        float visibleScale = (width - mXOri) / interval;
        switch (showPositionType) {
            ...
            case MIDDLE:
                if (mXData.size() <= visibleScale) {
                    ...
                    mSelectMiddle = middleIndex((int)visibleScale);
                }
            ...
            case END:
                if (mXData.size() <= visibleScale) {
                    ...
                    mSelectMiddle = (int)visibleScale;
                }
                return visibleScale;
            default:
                mSelectMiddle = 0;
    ...
    public void setValue(Map<Float, Float> value) {
        this.mSpinnerValue = value;
        invalidate();
    }

    public void setValue(Map<Float, Float> value, List<Float> xValue, List<Float> yValue,int flag,int flag_2) {
        this.mSpinnerValue = value;
        this.mXData = xValue;
        this.mYData = yValue;
        this.flag=flag;
    ...
    public Map<Float, Float> getValue() {
    ...
    private Point getSpinnerPoint(int valueIndex) {
        ...
        float x = (float)(mXInit +mXData.get(valueIndex)*interval);
        float y=0;
        if(flag==1) {
            y = mYOri - mYOri / 2 + dpToPx(18) - mYOri * (1 - 0.1f) * mSpinnerValue.get(mXData.get(valueIndex)) / mYData.get(mYData.size() - 1) / 2;
        }
        else if(flag==2){
            y = mYOri - mYOri / 6 + dpToPx(5) - mYOri * (1 - 0.1f) * mSpinnerValue.get(mXData.get(valueIndex)) / mYData.get(mYData.size() - 1)/6*5 ;
        }
        else{
            y = mYOri - mYOri * (1 - 0.1f) * mSpinnerValue.get(mXData.get(valueIndex)) / mYData.get(mYData.size() - 1) ;
        }
